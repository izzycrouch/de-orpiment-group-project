...
def create_fact_purchase_order(dim_date_df: pd.DataFrame, 
    ...
    df = pd.merge(df, dim_location_df, left_on='agreed_delivery_location_id', right_on='location_id', how='left').drop('location_id', axis=1)
    ...
    df = pd.merge(df, dim_date_df, left_on='created_date', right_on='location_id', how='left').drop('location_id', axis=1)
